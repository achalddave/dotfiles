...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Display the last K lines from stdin, updating every T seconds."
    )
    parser.add_argument(
        "-n", "--num-lines", type=int, default=5, help="Number of lines to display"
    )
    parser.add_argument(
        "-s", "--speed", type=int, default=10, help="Speed in frames per second"
    )
